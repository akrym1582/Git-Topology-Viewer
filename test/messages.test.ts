...
describe('isWebviewRequest', () => {
  it('accepts complete requests', () => {
    ...
    expect(
      isWebviewRequest({
        type: 'showCommitGroupDetails',
        commits: ['f41acde1234567890', 'e93b2101234567890'],
      }),
    ).toBe(true);
    ...
    expect(
      isWebviewRequest({
        type: 'contextMenu',
        nodeType: 'branch',
        nodeId: 'refs/heads/topic',
        selectedRefs: ['refs/heads/main', 'refs/heads/topic'],
        x: 10,
        y: 20,
      }),
    ).toBe(true);
  ...
  it('rejects malformed and stale requests', () => {
    ...
    expect(isWebviewRequest({ type: 'openDiff', left: 'main', right: 'base', path: 'file' })).toBe(
      false,
    );
    ...
    expect(
      isWebviewRequest({
        type: 'showCommitGroupDetails',
        commits: ['f41acde1234567890', 'f41acde1234567890'],
      }),
    ).toBe(false);
    expect(
      isWebviewRequest({
    ...
    expect(
      isWebviewRequest({
        ...
        nodeType: 'branch',
        nodeId: 'refs/heads/topic',
        selectedRefs: ['refs/heads/main', 'refs/heads/main'],
        x: 10,
        y: 20,
      }),
    ).toBe(false);
