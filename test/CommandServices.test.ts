...
describe('mutating Git command services', () => {
  ...
  it('constructs stash and commit commands', async () => { const {run,git}=mock(); const stash=new StashOperationService(git), commit=new CommitOperationService(git); await stash.apply('stash@{0}'); await stash.pop('stash@{0}'); await stash.drop('stash@{0}'); await commit.cherryPick('abc'); await commit.revert('def'); expect(run.mock.calls.map(call=>call[0])).toEqual([['stash','apply','stash@{0}'],['stash','pop','stash@{0}'],['stash','drop','stash@{0}'],['cherry-pick','abc'],['revert','def']]); });
