...
describe('BranchStatusService', () => {
  it('loads upstream ahead and behind state through Git', async () => {
    ...
    await expect(service.load([
      ...
      { name: 'origin/topic', fullName: 'refs/remotes/origin/topic', type: 'remoteBranch', commitId: 'remote' }
    ])).resolves.toEqual([{ ref: 'refs/heads/topic', local: true, remote: true, upstream: 'origin/topic', ahead: 3, behind: 2 }]);
    expect(run).toHaveBeenNthCalledWith(2, ['rev-list', '--left-right', '--count', 'refs/heads/topic...origin/topic']);
  });
});
