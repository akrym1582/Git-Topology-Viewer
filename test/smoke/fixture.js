...
const commitGraph = {
  nodes: [
    ...
    { id: commits.release, lane: 0, row: 5, x: 70, y: 390, refs: [refs[1], refs[4], refs[5]] },
    { id: commits.develop, lane: 0, row: 6, x: 70, y: 454, refs: [refs[3]] }
  ],
  edges: [
    ...
    { from: commits.feature, to: 'b16a9821234567890' }, { from: 'b16a9821234567890', to: 'c16a9821234567890' }, { from: 'c16a9821234567890', to: commits.release }, { from: commits.release, to: commits.develop }
  ]
};

function dispatchGraph(tags = true, remotes = false, includeCommitGraph = true, includeSignificantGraph = true) { const visible = nodeRefs => nodeRefs.filter(item => item.type === 'localBranch' || (item.type === 'tag' && tags) || (item.type === 'remoteBranch' && remotes)); const filteredGraph = { ...graph, nodes: graph.nodes.map(node => ({ ...node, refs: visible(node.refs) })).filter(node => node.refs.length) }; const filteredCommitGraph = { ...commitGraph, nodes: commitGraph.nodes.map(node => ({ ...node, refs: visible(node.refs) })) }; const significantGraph = { nodes: filteredCommitGraph.nodes.filter(node => node.refs.length || node.id === 'e93b2101234567890').concat({ id: 'commit-group:b16a9821234567890:c16a9821234567890', lane: 1, row: 3, x: 250, y: 262, refs: [], commitIds: ['b16a9821234567890', 'c16a9821234567890'] }), edges: filteredCommitGraph.edges.filter(edge => edge.from !== 'b16a9821234567890' && edge.to !== 'b16a9821234567890' && edge.from !== 'c16a9821234567890' && edge.to !== 'c16a9821234567890').filter(edge => !(edge.from === commits.feature && edge.to === commits.release)).concat({ from: commits.feature, to: 'commit-group:b16a9821234567890:c16a9821234567890' }, { from: 'commit-group:b16a9821234567890:c16a9821234567890', to: commits.release }) }; const payload = { repository: 'commerce-platform', currentBranch: 'main', branchStatuses: [{ ref: 'refs/heads/main', local: true, remote: false }, { ref: 'refs/heads/feature/login', local: true, remote: false }, { ref: 'refs/heads/develop', local: true, remote: false }], refs, graph: filteredGraph, ...(includeSignificantGraph ? { significantGraph } : {}) }; if (includeCommitGraph) payload.commitGraph = filteredCommitGraph; window.dispatchEvent(new MessageEvent('message', { data: { type: 'graph', payload } })); }
