...
function dispatchGraph(tags = true, remotes = false, includeCommitGraph = true) { const visible = nodeRefs => nodeRefs.filter(item => item.type === 'localBranch' || (item.type === 'tag' && tags) || (item.type === 'remoteBranch' && remotes)); const filteredGraph = { ...graph, nodes: graph.nodes.map(node => ({ ...node, refs: visible(node.refs) })).filter(node => node.refs.length) }; const filteredCommitGraph = { ...commitGraph, nodes: commitGraph.nodes.map(node => ({ ...node, refs: visible(node.refs) })) }; const significantGraph = { nodes: filteredCommitGraph.nodes.filter(node => node.refs.length || node.id === 'e93b2101234567890'), edges: filteredCommitGraph.edges.filter(edge => edge.from !== 'b16a9821234567890' && edge.to !== 'b16a9821234567890').concat({ from: commits.feature, to: commits.release }) }; const payload = { repository: 'commerce-platform', currentBranch: 'main', branchStatuses: [{ ref: 'refs/heads/main', local: true, remote: false }, { ref: 'refs/heads/feature/login', local: true, remote: false }, { ref: 'refs/heads/develop', local: true, remote: false }], refs, graph: filteredGraph, significantGraph }; if (includeCommitGraph) payload.commitGraph = filteredCommitGraph; window.dispatchEvent(new MessageEvent('message', { data: { type: 'graph', payload } })); }
