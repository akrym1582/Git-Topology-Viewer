...
function buildMinimalGraph(graph) { const hidden = new Set(['b16a9821234567890', 'c16a9821234567890']); return { nodes: graph.nodes.filter(node => node.refs.length || node.id === 'e93b2101234567890'), edges: graph.edges.filter(edge => !hidden.has(edge.from) && !hidden.has(edge.to)).filter(edge => !(edge.from === commits.feature && edge.to === commits.release)).concat({ from: commits.feature, to: commits.release }) }; }
