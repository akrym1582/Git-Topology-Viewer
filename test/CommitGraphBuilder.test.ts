...
const ref = (name: string, type: GitRef['type'], commitId: string): GitRef => ({ name, fullName: `refs/heads/${name}`, type, commitId });
...
describe('CommitGraphBuilder', () => {
  it('renders every loaded commit and both parents of a merge on distinct lanes', () => {
    ...
    expect(view.edges).toEqual([
      { from: 'main', to: 'merge' }, { from: 'merge', to: 'trunk' }, { from: 'merge', to: 'feature' },
      { from: 'feature', to: 'base' }, { from: 'trunk', to: 'base' }
    ]);
    expect(view.nodes.find(node => node.id === 'feature')?.lane).not.toBe(view.nodes.find(node => node.id === 'trunk')?.lane);
  });

  it('keeps commit nodes when their refs are hidden', () => {
    const nodes = new Map();
    nodes.set('head', { id: 'head', parents: [], refs: [ref('v1', 'tag', 'head')] });
    const view = new CommitGraphBuilder().build({ nodes, order: ['head'] }, { tags: false, remotes: false });
