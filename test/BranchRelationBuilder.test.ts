...
const ref = (name: string, type: GitRef['type'], commitId: string): GitRef => ({
  name,
  fullName: `${type === 'tag' ? 'refs/tags' : type === 'remoteBranch' ? 'refs/remotes' : 'refs/heads'}/${name}`,
  type,
  commitId,
});
...
describe('BranchRelationBuilder', () => {
  it('shows only ref groups and connects them across hidden commits', () => {
    ...
    const view = new BranchRelationBuilder().build({
      nodes,
      order: ['main', 'middle-2', 'middle-1', 'feature'],
    });
  ...
  it('keeps only the nearest relation after a merge', () => {
    ...
    const view = new BranchRelationBuilder().build({
      nodes,
      order: ['main', 'merge', 'feature', 'trunk', 'base'],
    });
    ...
    expect(view.nodes.find((node) => node.id === 'main')?.lane).not.toBe(
      view.nodes.find((node) => node.id === 'feature')?.lane,
    );
  });
  ...
  it('groups refs sharing a commit and applies tag and remote visibility', () => {
    ...
    nodes.set('head', {
      ...
      parents: ['base'],
      refs: [
        ref('main', 'localBranch', 'head'),
        ref('v1', 'tag', 'head'),
        ref('origin/main', 'remoteBranch', 'head'),
      ],
    });
    nodes.set('base', { id: 'base', parents: [], refs: [ref('base', 'localBranch', 'base')] });
    ...
    expect(
      localOnly.nodes.find((node) => node.id === 'head')?.refs.map((item) => item.name),
    ).toEqual(['main']);
    expect(allRefs.nodes.find((node) => node.id === 'head')?.refs.map((item) => item.name)).toEqual(
      ['main', 'v1', 'origin/main'],
    );
  });
});
