...
export class DiffService {
  ...
  async commitSummaries(commits: string[]): Promise<CommitInfo[]> {
    ...
    return commits
      .map((commit) => summaries.get(commit))
      .filter((value): value is CommitInfo => Boolean(value));
  }
  async branchLog(
    ref: string,
    base?: string,
    limit = 100,
  ): Promise<import('../domain/models').RefLog> {
    if (!base) return { ref, commits: await this.log(ref, limit) };
    const bases = (await this.git.run(['merge-base', '--all', ref, base]))
      .trim()
      .split('\n')
      .filter(Boolean);
  ...
  private parseFiles(out: string): ChangedFile[] {
    ...
    for (let i = 0; i < tokens.length;) {
      ...
      if (/^[RC]/.test(status)) {
        ...
        files.push({ status: status[0], oldPath, path: tokens[i++] });
  ...
  private parseStats(out: string): Array<{ additions: number; deletions: number }> {
    ...
    const stats: Array<{ additions: number; deletions: number }> = [];
    for (let i = 0; i < tokens.length;) {
  ...
  async commitDetails(commit: string): Promise<CommitDetails> {
    ...
    if (!commitInfo)
      throw new Error('The selected commit could not be read. Refresh the viewer and try again.');
  ...
  async compare(
    left: string,
    right: string,
    mode: 'divergence' | 'snapshot',
  ): Promise<BranchComparison> {
    const [bases, onlyLeft, onlyRight] = await Promise.all([
      this.git.run(['merge-base', '--all', left, right]),
      this.commits(`${right}..${left}`),
      this.commits(`${left}..${right}`),
    ]);
    ...
    const [names, numstat] = await Promise.all([
      this.git.run(['diff', '--name-status', '-z', range]),
      this.git.run(['diff', '--numstat', '-z', range]),
    ]);
    ...
    return {
      ...
      right,
      mode,
      mergeBases: bases.trim().split('\n').filter(Boolean),
      ahead: onlyLeft.length,
      behind: onlyRight.length,
      additions,
      deletions,
      files,
      onlyLeft,
  ...
  show(ref: string, path: string): Promise<string> {
    return this.git.run(['show', `${ref}:${path}`]);
  }
}
