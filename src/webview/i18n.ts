export interface WebviewStrings {
  relationGraph: string; significantGraph: string; refresh: string; tags: string; remoteBranches: string; hideDetails: string; showDetails: string; resizeDetails: string;
  commitGraph: string; commitGraphUnavailable: string; nodesAndRefs(nodes: number, refs: number): string; commitsAndRefs(commits: number, refs: number): string; readingRelations: string; zoomControls: string; zoomOut: string; resetZoom: string; zoomIn: string; branchAriaLabel(name: string): string; selectionHint: string;
...
const english: WebviewStrings = {
  relationGraph: 'Git Relations', significantGraph: 'Branches & merges', commitGraph: 'Commit history', commitGraphUnavailable: 'Commit history is unavailable. Refresh the viewer and try again.', refresh: 'Refresh', tags: 'Tags', remoteBranches: 'Remote branches', hideDetails: 'Hide details', showDetails: 'Show details', resizeDetails: 'Resize details pane',
  nodesAndRefs: (nodes, refs) => `${nodes} ref groups · ${refs} refs`, commitsAndRefs: (commits, refs) => `${commits} commits · ${refs} refs`, readingRelations: 'Reading branch relations…', zoomControls: 'Graph zoom controls', zoomOut: 'Zoom out', resetZoom: 'Reset zoom', zoomIn: 'Zoom in', branchAriaLabel: name => `${name} branch`, selectionHint: 'Click to select · Ctrl/Cmd-click two refs to compare · Right-click for comparison and branch actions',
...
const japanese: WebviewStrings = {
  relationGraph: 'Git 関係図', significantGraph: '分岐・マージ', commitGraph: 'コミット履歴', commitGraphUnavailable: 'コミット履歴を読み込めません。ビューアーを更新してからもう一度試してください。', refresh: '更新', tags: 'タグ', remoteBranches: 'リモートブランチ', hideDetails: '詳細を隠す', showDetails: '詳細を表示', resizeDetails: '詳細ペインの幅を変更',
  nodesAndRefs: (nodes, refs) => `${nodes} 個の参照グループ · ${refs} 個の参照`, commitsAndRefs: (commits, refs) => `${commits} 件のコミット · ${refs} 個の参照`, readingRelations: 'ブランチの関係を読み込み中…', zoomControls: 'グラフのズーム操作', zoomOut: '縮小', resetZoom: 'ズームをリセット', zoomIn: '拡大', branchAriaLabel: name => `${name} ブランチ`, selectionHint: 'クリックで選択 · Ctrl/Cmd キーを押しながら 2 つの参照をクリックして比較 · 右クリックで比較とブランチ操作を表示',
