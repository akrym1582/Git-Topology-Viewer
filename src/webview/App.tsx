...
export function App() {
  ...
  const visibleGraph = viewMode === 'relations' ? data?.graph : viewMode === 'significant' ? data?.significantGraph : data?.commitGraph;
  const renderGraph = visibleGraph ?? data?.graph;
  const renderMode = visibleGraph ? viewMode : 'relations';
  ...
  const selectRef = (ref: GitRef, additive: boolean) => {
    setComparison(undefined); setRefLog(undefined); setCommitDetails(undefined);
        ...
        {graph.nodes.map(node => {
          ...
          if (mode !== 'relations') {
            ...
            return commitNode.commitIds
              ? <CommitGroupNode key={node.id} node={commitNode} ui={ui}/>
              : <CommitNode key={node.id} node={commitNode} data={data} ui={ui} selected={selected} onSelect={onSelect} onContextMenu={onContextMenu}/>;
          }
...
function CommitGroupNode({ node, ui }: { node: CommitViewGraph['nodes'][number]; ui: WebviewStrings }) {
  const commitIds = node.commitIds ?? [];
  const first = commitIds[0]?.slice(0, 8) ?? '';
  const last = commitIds.at(-1)?.slice(0, 8) ?? '';
  return <g className="node commit-group" transform={`translate(${node.x},${node.y})`} aria-label={ui.commitGroupAriaLabel(commitIds.length)}><circle r="7"/><rect x="15" y="-20" width="220" height="40" rx="5"/><text className="group-label" x="27" y="-3">{ui.commitGroup(commitIds.length)}</text><text className="group-range" x="27" y="12">{first} → {last}</text></g>;
}
