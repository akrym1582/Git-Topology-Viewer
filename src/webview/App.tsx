...
export function App() {
  ...
  const hasSelection = selected.length > 0 || Boolean(selectedCommit) || Boolean(selectedCommitGroup);
...
function Graph({ graph, mode, data, ui, selected, selectedCommit, selectedCommitGroup, onSelect, onSelectCommit, onSelectCommitGroup, onContextMenu, onContextMenuCommit }: { graph: RefViewGraph | CommitViewGraph; mode: ViewMode; data: GraphPayload; ui: WebviewStrings; selected: Set<string>; selectedCommit?: string; selectedCommitGroup?: string[]; onSelect: (ref: GitRef, additive: boolean) => void; onSelectCommit: (commit: string) => void; onSelectCommitGroup: (commitIds: string[]) => void; onContextMenu: (ref: GitRef, event: MouseEvent) => void; onContextMenuCommit: (commit: string, event: MouseEvent) => void }) {
        ...
        {graph.nodes.map(node => {
          ...
          if (mode !== 'relations') {
            ...
            return commitNode.commitIds
              ? <CommitGroupNode key={node.id} node={commitNode} ui={ui} selected={sameCommitGroup(commitNode.commitIds, selectedCommitGroup)} onSelect={onSelectCommitGroup}/>
...
function CommitGroupNode({ node, ui, selected, onSelect }: { node: CommitViewGraph['nodes'][number]; ui: WebviewStrings; selected: boolean; onSelect: (commitIds: string[]) => void }) {
  ...
  return <g className={`node commit-group ${selected ? 'selected' : ''}`} transform={`translate(${node.x},${node.y})`} role="button" aria-pressed={selected} tabIndex={0} aria-label={ui.commitGroupAriaLabel(commitIds.length)} onClick={activate} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); activate(); } }}><rect className="group-badge" x="-52" y="-15" width="104" height="30" rx="15"/><text className="group-label" x="0" y="4" textAnchor="middle">{ui.commitGroup(commitIds.length)}</text><text className="group-range" x="64" y="4">{first} → {last}</text></g>;
}

function sameCommitGroup(left: string[] | undefined, right: string[] | undefined): boolean { return Boolean(left && right && left.length === right.length && left.every((commit, index) => commit === right[index])); }
