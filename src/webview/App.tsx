...
interface ContextMenuState {
  nodeType: 'branch' | 'remoteBranch' | 'tag' | 'commit';
  nodeId: string;
  ref?: GitRef;
  selectedRefs: string[];
  x: number;
...
export function App() {
  ...
  useEffect(() => {
    const receive = (event: MessageEvent) => {
      ...
      if (message.type === 'commitGroupDetails') {
        setCommitGroupSummaries(message.payload.commits);
        setCommitDetails({});
        setExpandedCommitDetails(new Set());
      }
      ...
      if (message.type === 'contextMenuItems') {
        ...
        if (message.nodeType === 'commit' || ref)
          setMenu({
            nodeType: message.nodeType,
            nodeId: message.nodeId,
            ref,
            selectedRefs: message.selectedRefs ?? (ref ? [ref.fullName] : []),
            x: message.x,
            y: message.y,
            items: message.items,
          });
      }
      if (message.type === 'focusRef' && message.commitId)
        document
          .querySelector(`[data-ref-node="${message.commitId}"]`)
  ...
  const visibleRefs = useMemo(
    () => renderGraph?.nodes.flatMap((node) => node.refs) ?? [],
    [renderGraph],
  );
  const visibleCommitCount = useMemo(
    () =>
      renderGraph?.nodes.reduce(
        (count, node) =>
          count + ((node as CommitViewGraph['nodes'][number]).commitIds?.length ?? 1),
        0,
      ) ?? 0,
    [renderGraph],
  );
  const hasSelection =
    selected.length > 0 || Boolean(selectedCommit) || Boolean(selectedCommitGroup);
  const detailsVisible = inspectorVisible && hasSelection;
  if (!data)
    return (
      <main className="loading">
        <div className="spinner" />
        <h2>{ui.readingRelations}</h2>
  ...
  if (!renderGraph)
    return (
      <main className="loading">
        <h2>{ui.graphUnavailable}</h2>
        {error && <p className="error">{error}</p>}
      </main>
    );
  const clampInspectorWidth = (width: number) =>
    Math.min(getMaxInspectorWidth(), Math.max(MIN_INSPECTOR_WIDTH, width));
  ...
  const selectRef = (ref: GitRef, additive: boolean) => {
    ...
    setSelectedCommitGroup(undefined);
    if (!additive) {
      setSelected([ref]);
  ...
  const selectCommit = (
    ...
    if (!keepCommitGroup) setSelectedCommitGroup(undefined);
    if (!keepRefLog) {
      setSelected([]);
      setRefLog(undefined);
    }
    if (!keepRefLog && !keepCommitGroup) {
      setCommitDetails({});
      setExpandedCommitDetails(new Set());
    }
  ...
  const selectCommitGroup = (commitIds: string[]) => {
    ...
    setCommitDetails({});
    setExpandedCommitDetails(new Set());
    setCommitGroupSummaries(undefined);
    setSelected([]);
    setSelectedCommit(undefined);
    setSelectedCommitGroup(commitIds);
    ...
    <div className="app" onContextMenu={(event) => event.preventDefault()}>
      <div className="filters">
        <label>
          <input
            type="checkbox"
            checked={tags}
            onChange={(event) => setRefVisibility(event.target.checked, remotes)}
          />{' '}
          {ui.tags}
        </label>
        <label>
          <input
            type="checkbox"
            checked={remotes}
            onChange={(event) => setRefVisibility(tags, event.target.checked)}
          />{' '}
          {ui.remoteBranches}
        </label>
        <label>
          <input
            type="checkbox"
            checked={showBranchCommits}
            onChange={(event) => setShowBranchCommits(event.target.checked)}
          />{' '}
          {ui.showBranchCommits}
        </label>
        <label>
          <input
            type="checkbox"
            checked={summarizeCommits}
            onChange={(event) => setSummarizeCommits(event.target.checked)}
            disabled={!showBranchCommits}
          />{' '}
          {ui.summarizeCommits}
        </label>
        <label>
          <input
            ...
            onChange={(event) => setShowAllCommits(event.target.checked)}
            disabled={!showBranchCommits}
          />{' '}
          {ui.showAllCommits}
        </label>
        ...
        >
          {detailsVisible ? ui.hideDetails : ui.showDetails}
        </button>
        <span>
          {showBranchCommits
            ? ui.commitsAndRefs(visibleCommitCount, visibleRefs.length)
            : ui.nodesAndRefs(renderGraph.nodes.length, visibleRefs.length)}
        </span>
        <button className="refresh" onClick={() => vscode.postMessage({ type: 'refresh' })}>
          ↻ {ui.refresh}
        </button>
      </div>
      <div
        className={`content ${detailsVisible ? '' : 'inspector-hidden'}`}
        style={
          detailsVisible
            ? { gridTemplateColumns: `minmax(0, 1fr) 9px ${inspectorWidth}px` }
            : undefined
        }
      >
        <Graph
          ...
          data={data}
          ui={ui}
          selected={new Set(selected.map((ref) => ref.fullName))}
          selectedCommit={selectedCommit}
          selectedCommitGroup={selectedCommitGroup}
          onSelect={selectRef}
          onSelectCommit={selectCommit}
          onSelectCommitGroup={selectCommitGroup}
        ...
        {detailsVisible && (
          <>
            <div
              ...
              aria-label={ui.resizeDetails}
              aria-valuemin={MIN_INSPECTOR_WIDTH}
              aria-valuemax={getMaxInspectorWidth()}
              aria-valuenow={inspectorWidth}
              tabIndex={0}
              title={ui.resizeDetails}
              onPointerDown={startInspectorResize}
              onKeyDown={(event) => {
                if (event.key === 'ArrowLeft') {
                  event.preventDefault();
                  adjustInspectorWidth(16);
                }
                ...
                if (event.key === 'End') {
                  event.preventDefault();
                  setInspectorWidth(getMaxInspectorWidth());
                }
              }}
            />
            <aside>
              <Inspector
                ...
                commitGroupSummaries={commitGroupSummaries}
                selectedCommit={selectedCommit}
                selectedCommitGroup={selectedCommitGroup}
                onSelectCommit={(commit) =>
                  selectCommit(commit, Boolean(refLog), Boolean(selectedCommitGroup))
                }
      ...
      {menu && (
        <ContextMenu
          ...
          menu={menu}
          onRun={(command) => {
            if (command === 'showChanges' && menu.nodeType === 'commit')
              selectCommit(menu.nodeId, Boolean(refLog), Boolean(selectedCommitGroup), false, true);
            else
              vscode.postMessage({
                ...
                nodeId: menu.nodeId,
                selectedRefs: menu.selectedRefs,
              });
            setMenu(undefined);
          }}
        />
      ...
      {notice && (
        <div className="toast success" role="status" onClick={() => setNotice('')}>
          {notice}
        </div>
      )}
    </div>
...
function getMaxInspectorWidth(): number {
  return Math.max(
    MIN_INSPECTOR_WIDTH,
    Math.min(MAX_INSPECTOR_WIDTH, window.innerWidth - MIN_GRAPH_WIDTH),
  );
}
...
function Graph({
  ...
  graph: RefViewGraph | CommitViewGraph;
  mode: ViewMode;
  data: GraphPayload;
  ui: WebviewStrings;
  selected: Set<string>;
  selectedCommit?: string;
  selectedCommitGroup?: string[];
  onSelect: (ref: GitRef, additive: boolean) => void;
  onSelectCommit: (commit: string) => void;
  onSelectCommitGroup: (commitIds: string[]) => void;
  ...
  const updateZoom = (next: number) =>
    setZoom(Math.min(2, Math.max(0.5, Math.round(next * 10) / 10)));
  return (
    <section
      className="canvas"
      onWheel={(event) => {
    ...
    >
      <div className="zoom-controls" aria-label={ui.zoomControls}>
        <button
        ...
        <button aria-label={ui.resetZoom} title={ui.resetZoom} onClick={() => setZoom(1)}>
          {Math.round(zoom * 100)}%
        </button>
        <button
        ...
        <g transform={`scale(${zoom})`}>
          <g className={`edges ${mode !== 'relations' ? 'commit-edges' : ''}`}>
            {graph.edges.map((edge, edgeIndex) => {
              ...
              if (mode === 'relations') {
                ...
                const fromPoint = {
                  x: from.x + REF_WIDTH / 2 - REF_ICON_CENTER_X,
                  y: from.y + fromLayout.top + REF_HEIGHT,
                };
                const toPoint = {
                  x: to.x + REF_WIDTH / 2 - REF_ICON_CENTER_X,
                  y: to.y + toLayout.top,
                };
                return (
                  <path
                    key={`${edge.from}:${edge.to}:${edgeIndex}`}
                    d={relationEdgePath(fromPoint, toPoint)}
                  />
                );
              }
              const fromX = from.x;
          ...
          {graph.nodes.map((node) => {
            ...
            if (mode !== 'relations') {
              ...
              return commitNode.commitIds ? (
                <CommitGroupNode
                  ...
                  node={commitNode}
                  ui={ui}
                  selected={sameCommitGroup(commitNode.commitIds, selectedCommitGroup)}
                  onSelect={onSelectCommitGroup}
                />
              ) : (
                <CommitNode
                  key={node.id}
            ...
            return (
              ...
              >
                {layout.refs.length > 1 && (
                  <g className="ref-connectors" aria-hidden="true">
                    {layout.refs.slice(1).map(({ ref, position }) => (
                      <path
                        key={ref.fullName}
                        data-ref-connector={ref.fullName}
                        d={`M ${layout.anchor.x} ${layout.anchor.y} V ${position.y - REF_ROW_GAP / 2} H ${position.x + REF_ICON_CENTER_X} V ${position.y + REF_ANCHOR_Y}`}
                      />
                    ))}
                  </g>
                )}
                {layout.refs.map(({ ref, position }) => (
                  <RefNode
                    key={ref.fullName}
                    gitRef={ref}
                    position={position}
...
const REF_TEXT_Y = 19;
const REF_ANCHOR_Y = REF_TEXT_Y - 5;
...
function layoutRefs(refs: GitRef[]): RefLayout {
  ...
  const anchor = positionedRefs[0]
    ? {
        x: positionedRefs[0].position.x + REF_ICON_CENTER_X,
        y: positionedRefs[0].position.y + REF_ANCHOR_Y,
      }
    : { x: 0, y: 0 };
  return { refs: positionedRefs, anchor, top: firstRowY };
}
...
function CommitNode({
  ...
  node: CommitViewGraph['nodes'][number];
  data: GraphPayload;
  ui: WebviewStrings;
  selected: Set<string>;
  selectedCommit?: string;
  ...
  return (
    <g
      data-ref-node={node.id}
      className={`node commit-node ${selectedCommit === node.id ? 'selected' : ''}`}
      transform={`translate(${node.x},${node.y})`}
      role="button"
      tabIndex={0}
      aria-label={ui.commitAriaLabel(node.id)}
      onClick={activate}
      onContextMenu={(event) => onContextMenuCommit(node.id, event)}
      onKeyDown={(event) => {
    ...
    >
      <circle r="7" />
      <text className="commit-id" x="15" y="4">
        {node.id.slice(0, 8)}
      </text>
      {refs.map(({ ref, position }) => (
        <RefNode
          key={ref.fullName}
          gitRef={ref}
          position={{ x: position.x + 16, y: position.y - 22 }}
          data={data}
          ui={ui}
          selected={selected}
          onSelect={onSelect}
...
function CommitGroupNode({
  ...
  return (
    <g
      className={`node commit-group ${selected ? 'selected' : ''}`}
      transform={`translate(${node.x},${node.y})`}
      role="button"
      aria-pressed={selected}
      tabIndex={0}
      aria-label={ui.commitGroupAriaLabel(commitIds.length)}
      onClick={activate}
      onKeyDown={(event) => {
        if (event.key === 'Enter' || event.key === ' ') {
          event.preventDefault();
    ...
    >
      <rect className="group-badge" x="-52" y="-15" width="104" height="30" rx="15" />
      <text className="group-label" x="0" y="4" textAnchor="middle">
        {ui.commitGroup(commitIds.length)}
      </text>
      <text className="group-range" x="64" y="4">
        {first} → {last}
      </text>
    </g>
  );
...
function sameCommitGroup(left: string[] | undefined, right: string[] | undefined): boolean {
  return Boolean(
    left &&
    right &&
    left.length === right.length &&
    left.every((commit, index) => commit === right[index]),
  );
}
...
function RefNode({
  ...
  return (
    <g
      className={`ref ${gitRef.type} ${current ? 'current' : ''} ${selected.has(gitRef.fullName) ? 'selected' : ''}`}
      transform={`translate(${position.x},${position.y})`}
      onClick={(event) => {
        event.stopPropagation();
        onSelect(gitRef, event.ctrlKey || event.metaKey);
      }}
      onContextMenu={(event) => onContextMenu(gitRef, event)}
      role="button"
      aria-label={`${ui.branchAriaLabel(gitRef.name)}${current ? ` (${ui.currentBranchMarker})` : ''}`}
    >
      <rect width={REF_WIDTH} height={REF_HEIGHT} rx="0" />
      <text className="ref-icon" x="14" y={REF_TEXT_Y} textAnchor="middle">
        {refIcon(gitRef)}
      </text>
      <text className="ref-name" x="24" y={REF_TEXT_Y}>
        {displayRefName(gitRef.name)}
        {branchState(data, gitRef, ui)}
      </text>
    </g>
  );
}
...
function Inspector({
  ...
  commitGroupSummaries?: CommitInfo[];
  selectedCommit?: string;
  selectedCommitGroup?: string[];
  onSelectCommit: (commit: string) => void;
  ...
  if (!primary && selectedCommitGroup)
    return (
      <CommitGroupInspector
        ui={ui}
        commitIds={selectedCommitGroup}
        summaries={commitGroupSummaries}
        details={commitDetails}
        expandedCommitDetails={expandedCommitDetails}
        selectedCommit={selectedCommit}
        onSelectCommit={onSelectCommit}
        onCloseCommitDetails={onCloseCommitDetails}
        onContextMenuCommit={onContextMenuCommit}
        onClose={onClose}
      />
    );
  if (!primary && selectedCommit)
    return (
      <CommitInspector
        ui={ui}
        commit={selectedCommit}
        details={commitDetails[selectedCommit]}
        expanded={expandedCommitDetails.has(selectedCommit)}
        onCloseCommitDetails={onCloseCommitDetails}
        onClose={onClose}
      />
    );
  ...
  return (
    <div className="inspector">
      <button
        className="close"
        aria-label={ui.closeInspector}
        title={ui.closeInspector}
        onClick={onClose}
      >
        ×
      </button>
      <small>
        {selected.length === 2
          ? ui.comparingRefs
          : primary.type.replace(/([A-Z])/g, ' $1').toUpperCase()}
      </small>
      <h2>{selected.length === 2 ? `${primary.name} ↔ ${selected[1].name}` : primary.name}</h2>
      <div className="actions">
        <button onClick={() => vscode.postMessage({ type: 'copy', value: primary.name })}>
          {ui.copyName}
        </button>
      </div>
      {selected.length === 1 ? (
        refLog?.ref === primary.fullName ? (
          <RefLogView
            ui={ui}
            value={refLog}
            details={commitDetails}
          ...
          <p className="loading-details">{ui.loadingRefHistory}</p>
        )
      ) : (
        <>
          <hr />
...
function CommitInspector({
  ...
  const value = details?.commit.id === commit ? details : undefined;
  return (
    <div className="inspector">
      <button
        className="close"
      ...
      <small>{ui.commitDetails}</small>
      <h2>
        <code>{commit}</code>
      </h2>
      {value ? (
        <>
          <p className="commit-subject">{value.commit.subject}</p>
          {expanded && (
            <CommitFiles ui={ui} value={value} onClose={() => onCloseCommitDetails(commit)} />
          )}
        </>
      ) : (
        <p className="loading-details">{ui.loadingCommitDetails}</p>
      )}
    </div>
  );
}
function CommitGroupInspector({
  ui,
  commitIds,
  summaries,
  details,
  ...
  ui: WebviewStrings;
  commitIds: string[];
  summaries?: CommitInfo[];
  details: Record<string, CommitDetails>;
  expandedCommitDetails: Set<string>;
  selectedCommit?: string;
  onSelectCommit: (commit: string) => void;
  onCloseCommitDetails: (commit: string) => void;
  ...
  const summaryMap = new Map((summaries ?? []).map((commit) => [commit.id, commit]));
  const commits = commitIds.map((id) => summaryMap.get(id) ?? { id, subject: '' });
  return (
    <div className="inspector">
      <button
        className="close"
      ...
      <small>{ui.commitGroupDetails}</small>
      <h2>{ui.commitGroup(commitIds.length)}</h2>
      <RefLogView
        ui={ui}
        value={{ ref: '', commits }}
        details={details}
...
function RefLogView({
  ...
  const entry = (commit: import('../domain/models').CommitInfo, branchPoint = false) => {
    const detailsForCommit = details[commit.id];
    const expanded = expandedCommitDetails.has(commit.id) && Boolean(detailsForCommit);
    return (
      <React.Fragment key={commit.id}>
        {branchPoint && <small className="branch-point">{ui.branchPoint}</small>}
        <button
          className={`log-entry ${selectedCommit === commit.id ? 'selected' : ''}`}
          aria-label={ui.showChangesFor(commit.id)}
          onClick={() => onSelectCommit(commit.id)}
          onContextMenu={(event) => {
            event.preventDefault();
            event.stopPropagation();
            onContextMenuCommit(commit.id, event);
          }}
        >
          <code>{commit.id.slice(0, 8)}</code>
          <span>
            <strong>{commit.subject || detailsForCommit?.commit.subject || ''}</strong>
            {commitMeta(ui, commit)}
          </span>
        </button>
        {expanded && (
          <CommitFiles
            ui={ui}
            value={detailsForCommit}
            onClose={() => onCloseCommitDetails(commit.id)}
          />
        )}
      </React.Fragment>
    );
  ...
  return (
    <div className="ref-log">
      <hr />
      <h3>{title}</h3>
      {value.commits.length === 0 ? (
        <p>{ui.noCommitsFound}</p>
      ) : (
        value.commits.map((commit) => entry(commit))
      )}
      {value.branchPoint && entry(value.branchPoint, true)}
    </div>
  );
}
function CommitFiles({
  ...
  return (
    <div className="commit-files">
      {onClose && (
        <button
          className="close-commit-files"
          aria-label={ui.closeCommitFiles}
          title={ui.closeCommitFiles}
          onClick={onClose}
        >
          ×
        </button>
      ...
      <div className="metrics">
        ...
        <div>
          <b className="added">+{value.additions}</b>
          <small>{ui.added}</small>
        </div>
        <div>
          <b className="removed">−{value.deletions}</b>
          <small>{ui.removed}</small>
        </div>
      </div>
      {value.files.length === 0 ? (
        <p>{ui.noFileChanges}</p>
      ) : (
        <div className="files">
          {value.files.map((file) => (
            <button
              ...
              onClick={() =>
                vscode.postMessage({
                  type: 'openDiff',
                  left: value.commit.id,
                  right: value.parent ?? value.commit.id,
                  path: file.path,
            ...
            >
              ...
              <span>{file.path}</span>
              <small>
                <ins>+{file.additions ?? 0}</ins> <del>−{file.deletions ?? 0}</del>
              </small>
            </button>
          ))}
        </div>
...
function branchState(data: GraphPayload, ref: GitRef, ui: WebviewStrings): string {
  ...
  const head = isCurrentBranch(data, ref) ? ` [${ui.currentBranchMarker}]` : '';
  const status = data.branchStatuses.find((item) => item.ref === ref.fullName);
  if (!status?.remote) return `${head} [L]`;
  const movement =
    status.ahead || status.behind
      ? `${status.ahead ? ` ↑${status.ahead}` : ''}${status.behind ? ` ↓${status.behind}` : ''}`
      : ' =';
  return `${head} [L][R]${movement}`;
}
function isCurrentBranch(data: GraphPayload, ref: GitRef): boolean {
  return ref.type === 'localBranch' && ref.name === data.currentBranch;
}
function commitMeta(ui: WebviewStrings, commit: CommitInfo): React.ReactNode {
  if (!commit.committer && !commit.date) return null;
  const date = commit.date ? formatCommitDate(commit.date) : '';
  const label = ui.commitMeta(commit.committer ?? '', date);
  return (
    <small className="commit-meta" title={commit.date}>
      {label}
    </small>
  );
}
...
function refIcon(ref: GitRef): string {
  return ref.type === 'tag' ? '◆' : ref.type === 'remoteBranch' ? '☁' : '⑂';
}
  ...
  return (
    ...
    >
      <div className="context-title">
        {menu.selectedRefs.length === 2
          ? ui.compareSelectedRefs
          : (menu.ref?.name ?? menu.nodeId.slice(0, 8))}
      </div>
      {groups.map((group) => {
        const items = menu.items.filter((item) => item.visible && item.group === group);
        return items.length ? (
          <React.Fragment key={group}>
            <div className="context-group">{ui.contextGroup(group)}</div>
            {items.map((item) => (
              <button
                key={item.command}
                role="menuitem"
