...
function CommitGroupInspector({ ui, commitIds, summaries, details, onSelectCommit, onClose }: { ui: WebviewStrings; commitIds: string[]; summaries?: CommitInfo[]; details?: CommitDetails; onSelectCommit: (commit: string) => void; onClose: () => void }) { const subjects = new Map((summaries ?? []).map(commit => [commit.id, commit.subject])); const commits = commitIds.map(id => ({ id, subject: subjects.get(id) ?? '' })); return <div className="inspector"><button className="close" aria-label={ui.closeInspector} title={ui.closeInspector} onClick={onClose}>×</button><small>{ui.commitGroupDetails}</small><h2>{ui.commitGroup(commitIds.length)}</h2><RefLogView ui={ui} value={{ ref: '', commits }} details={details} onSelectCommit={onSelectCommit} title={ui.commitGroupDetails}/></div>; }
