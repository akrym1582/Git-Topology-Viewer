...
export function App() {
  ...
  useEffect(() => {
    const receive = (event: MessageEvent) => {
      ...
      if (message.type === 'commitGroupDetails') setCommitGroupSummaries(message.payload.commits);
  ...
  const selectCommitGroup = (commitIds: string[]) => {
    setComparison(undefined); setRefLog(undefined); setCommitDetails(undefined); setCommitGroupSummaries(undefined); setSelected([]); setSelectedCommit(undefined); setSelectedCommitGroup(commitIds);
...
function CommitGroupInspector({ ui, commitIds, summaries, details, onSelectCommit, onContextMenuCommit, onClose }: { ui: WebviewStrings; commitIds: string[]; summaries?: CommitInfo[]; details?: CommitDetails; onSelectCommit: (commit: string) => void; onContextMenuCommit: (commit: string, event: MouseEvent) => void; onClose: () => void }) { const summaryMap = new Map((summaries ?? []).map(commit => [commit.id, commit])); const commits = commitIds.map(id => summaryMap.get(id) ?? ({ id, subject: '' })); return <div className="inspector"><button className="close" aria-label={ui.closeInspector} title={ui.closeInspector} onClick={onClose}>×</button><small>{ui.commitGroupDetails}</small><h2>{ui.commitGroup(commitIds.length)}</h2><RefLogView ui={ui} value={{ ref: '', commits }} details={details} onSelectCommit={onSelectCommit} onContextMenuCommit={onContextMenuCommit} title={ui.commitGroupDetails}/></div>; }
function RefLogView({ ui, value, details, onSelectCommit, onContextMenuCommit, title = ui.commitHistory }: { ui: WebviewStrings; value: RefLog; details?: CommitDetails; onSelectCommit: (commit: string) => void; onContextMenuCommit: (commit: string, event: MouseEvent) => void; title?: string }) { const entry = (commit: import('../domain/models').CommitInfo, branchPoint = false) => <React.Fragment key={commit.id}>{branchPoint && <small className="branch-point">{ui.branchPoint}</small>}<button className={`log-entry ${details?.commit.id === commit.id ? 'selected' : ''}`} aria-label={ui.showChangesFor(commit.id)} onClick={() => onSelectCommit(commit.id)} onContextMenu={event => { event.preventDefault(); event.stopPropagation(); onContextMenuCommit(commit.id, event); }}><code>{commit.id.slice(0, 8)}</code><span><strong>{commit.subject || (details?.commit.id === commit.id ? details.commit.subject : '')}</strong>{commitMeta(ui, commit)}</span></button>{details?.commit.id === commit.id && <CommitFiles ui={ui} value={details}/>}</React.Fragment>; return <div className="ref-log"><hr/><h3>{title}</h3>{value.branchPoint && entry(value.branchPoint, true)}{value.commits.length === 0 ? <p>{ui.noCommitsFound}</p> : value.commits.map(commit => entry(commit))}</div>; }
