...
export interface GraphContextMenuItem {
  ...
  enabled: boolean;
  visible: boolean;
  group: 'compare' | 'graph' | 'git' | 'manage' | 'copy';
}
...
const menuCommands: GraphMenuCommand[] = [
  'compareCurrent',
  'selectCompareBase',
  'compareBase',
  'compareWith',
...
export function isWebviewRequest(value: unknown): value is WebviewRequest {
  ...
  switch (value.type) {
    ...
    case 'contextMenu':
      return (
        (value.nodeType === 'commit'
          ? isCommitId(value.nodeId) &&
            Array.isArray(value.selectedRefs) &&
            value.selectedRefs.length === 0
          : (value.nodeType === 'branch' ||
              value.nodeType === 'remoteBranch' ||
              value.nodeType === 'tag') &&
            isString(value.nodeId) &&
            isRefSelection(value.selectedRefs)) &&
      ...
      return (
        (value.nodeType === 'commit'
          ...
          : (value.nodeType === 'branch' ||
              ...
              value.nodeType === 'tag') &&
            isString(value.nodeId)) &&
        (value.selectedRefs === undefined ||
          (value.nodeType === 'commit'
            ? Array.isArray(value.selectedRefs) && value.selectedRefs.length === 0
            : isRefSelection(value.selectedRefs))) &&
