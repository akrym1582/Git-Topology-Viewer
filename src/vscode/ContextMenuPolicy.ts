...
export class ContextMenuPolicy {
  comparisonItems(t: Translate = (message) => message): GraphContextMenuItem[] {
    const item = (command: GraphMenuCommand, label: string): GraphContextMenuItem => ({
      command,
      label,
      group: 'compare',
      enabled: true,
  ...
  branchItems(context: MenuContext, t: Translate = (message) => message): GraphContextMenuItem[] {
    ...
    const item = (
      ...
      command,
      label,
      group: command.startsWith('delete') ? 'manage' : 'git',
      enabled,
      visible: true,
    });
    if (!normal) {
      if (operation.type === 'rebasing')
        return [
          item('continueRebase', t('Continue Rebase'), operation.hasConflicts === false),
          item('abortRebase', t('Abort Rebase')),
        ];
      if (operation.type === 'cherryPicking')
        return [
          item('continueCherryPick', t('Continue Cherry-pick'), operation.hasConflicts === false),
          item('abortCherryPick', t('Abort Cherry-pick')),
        ];
      return [];
    }
    if (remote)
      return [
        item('checkoutRemote', t('Checkout as Local Branch…')),
        item('fetch', t('Fetch')),
        item('deleteRemote', t('Delete Remote Branch…')),
      ];
    if (!local) return [];
    return [
      item('push', hasUpstream ? t('Push') : t('Push and Set Upstream…')),
      item('pull', t('Pull'), current),
      item('fetch', hasUpstream ? t('Fetch') : t('Fetch…')),
      item('mergeIntoCurrent', t('Merge into Current Branch…'), !current && Boolean(currentBranch)),
      item(
        'rebaseCurrentOnto',
        t('Rebase Current Branch onto This…'),
        !current && Boolean(currentBranch),
      ),
