...
export class CommitGraphBuilder {
  build(
    ...
    const edges = graph.order.flatMap((from) =>
      graph.nodes
        .get(from)!
        .parents.filter((to) => visible.has(to))
        .map((to) => ({ from, to })),
    );
    return { nodes, edges };
  }
  ...
  private assignLanes(graph: CommitGraph): Map<string, number> {
    ...
    for (const id of graph.order) {
      ...
      const parents = [
        ...new Set(graph.nodes.get(id)!.parents.filter((parent) => graph.nodes.has(parent))),
      ];
  ...
  private replaceWithParents(
    ...
    active[lane] =
      first && !active.some((value, index) => index !== lane && value === first)
        ? first
        : undefined;
